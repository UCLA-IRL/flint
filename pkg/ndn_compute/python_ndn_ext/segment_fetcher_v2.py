...
from ndn.types import InterestNack, MetaInfo, BinaryStr


async def fetch_segments_with_retry(app: NDNApp, name: Name, validator: Validator,
                                    must_be_fresh: bool = False, lifetime: int = 6000,
                                    initial_backoff: Optional[list[int]] = None) -> AsyncGenerator[Optional[BinaryStr], None]:
    """
    Syntactic sugar to fetch segmented data with initial retry.
    ...
    :param initial_backoff: Backoff pattern (in seconds) to wait for the first segment to become available.
        (default [0, 0.5, 0.5, 1, 2, 3, 4, 5, 10, 20, 30])
    """
    if initial_backoff is None:
        initial_backoff = [0, 0.5, 0.5, 1, 2, 3, 4, 5, 10, 20, 30]
    ...
    for sleep_time in initial_backoff:
        ...
        try:
            async for content in fetch_segments(app, name, validator, must_be_fresh, lifetime):
    ...
    if not first_segment_available:
        raise Exception("fetch_segments failed, exhausted attempts to fetch the first segment")


async def fetch_segments(app: NDNApp, name: Name, validator: Validator, must_be_fresh: bool = False,
                         lifetime: int = 6000) -> AsyncGenerator[Optional[BinaryStr], None]:
    """
